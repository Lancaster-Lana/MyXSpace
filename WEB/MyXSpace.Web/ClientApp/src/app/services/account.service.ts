...
export class AccountService {
  ...
  get currentTenant() : Observable<Tenant>
  {
    return this.accountEndpoint.getTenantEndpoint();
  }
  ...
  updateUser(user: UserEdit)
  {
    ...
    let observUser = this.accountEndpoint.getUserByUserNameEndpoint<User>(user.userName);
    observUser.subscribe(u => user.id = u.id);
  ...
  updateRole(role: Role) {
    ...
    if (role.roleId) {
      return this.accountEndpoint.getUpdateRoleEndpoint(role, role.roleId).pipe(
      ...
      return this.accountEndpoint.getRoleByRoleNameEndpoint<Role>(role.name).pipe(
        mergeMap(foundRole => {
          ...
          console.log("Update role, found id: " + role.roleId); 
          return this.accountEndpoint.getUpdateRoleEndpoint(role, role.roleId)
        }),
        tap(data => this.onRolesChanged([role], AccountService.roleModifiedOperation)));
    }
  }
  ...
  deleteRole(roleOrRoleId: string | Role): Observable<Role> {
    ...
    if (typeof roleOrRoleId === 'string' || roleOrRoleId instanceof String) {
      ...
      if (roleOrRoleId.roleId) {
        return this.deleteRole(roleOrRoleId.roleId);
      }
      else {
        return this.accountEndpoint.getRoleByRoleNameEndpoint<Role>(roleOrRoleId.name).pipe<Role>(
          mergeMap(role => this.deleteRole(role.roleId)));
      }
    }
  }
