...
export class AccountEndpoint extends EndpointFactory {
  ...
  private readonly _currentTenantUrl: string = "/api/account/tenant"; //"/api/tenants"; //
  ...
  //private readonly _registerUrl: string = "/api/account/register" ///user/register";
  ...
  getTenantEndpoint(): Observable<Tenant>
  {
    //return this.http.get<Tenant>(endpointUrl, this.getRequestHeaders()).pipe<Tenant>( catchError(error => {return this.handleError(error, () => this.getTenantEndpoint());}));

    return this.http.get<Tenant>(this._currentTenantUrl, this.getRequestHeaders());
  }
  ...
  getRoleEndpoint<T>(roleId: string): Observable<T>
  {
    ...
    console.log('getRoleEndpoint ID' + roleId);
  ...
  {
    console.log('getUpdateRoleEndpoint ID' + roleId);
    let endpointUrl = `${this.rolesUrl}/${roleId}`;
