...
export class UserInfoComponent implements OnInit {
  ...
  private userRoles: Role[] = []; //Current user roles
  ...
  @ViewChild('f', { read: NgForm, static: false })
  form: NgForm;
  ...
  private onCurrentUserDataLoadSuccessful(user: User, roles: Role[])
  {
    ...
    this.user = user;
    //this.userRoles = this.allRoles.filter(r => this.user.roles.includes(r.name)); //roles for DropDown
  ...
  private save() {
    ...
    //TOOD: 
    this.userEdit.roles = this.userRoles.map(r => r.name);//this.selectedRoles.map(r => r.name);

    if (this.isNewUser) {
      this.accountService.newUser(this.userEdit)
        .subscribe(user => this.saveSuccessHelper(user),
  ...
  private setRoles(user: User, allRoles?: Role[]) {
    ...
    if (user.roles) {
        ...
        this.userRoles = this.allRoles.filter(role => user.roles.includes(role.name)); //roles for DropDown
    }
