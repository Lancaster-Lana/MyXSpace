...
namespace MyXSpace.WebSPA.Api
{
    ...
    {
        private readonly SignInManager<AppUser> _signInManager; //TODO: separate SigninManager to login AppUser into specific Tenant
        ...
        {
            //TODO: replace with 
            /*var authTicket = aipServer.TryLogin(request);
            if(authTicket != null)
            {
                //connect to MyX SPA via auth token (that requested from AIP) 
                var resultSign = SignIn(authTicket.Principal, authTicket.Properties, authTicket.AuthenticationScheme);
                return resultSign;
            }*/
            ...
            if (request.IsPasswordGrantType())
            {
                ...
                var ticket = await CreateTicketAsync(request, user); //TODO: ticket = aipService.TryLogin(request, user)

                var resultCreateAndSign = SignIn(ticket.Principal, ticket.Properties, ticket.AuthenticationScheme);

                return resultCreateAndSign;
            }
